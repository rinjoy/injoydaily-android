...
export default class WeaklyVideoSeeAll extends React.Component {
    ...
    async getDailyInspirationApiData(challangeId, featureId, categoryId, week, uid) {
      // alert(categoryId)
        const token_ = await SecureStore.getItemAsync('token');
        const url = global.base_url_live+'v1/api/get-weekly-video-all';
    ...
    InheritedData(data) {
        if (data.status == true) {
            this.setDataList(data.weekly_video_all)
        }
    }
    ...
    renderRow(dataHolder, index) {
        //console.log('renderRow',dataHolder);
        ...
        return (
            <View
                key={index}
                style={{marginTop: 20, marginBottom: 20, backgroundColor: 'transparent', width: '100%'}}>
                <View style={{paddingHorizontal: 15}}>
                <Text style={{color: 'gray', fontFamily: 'PoppinsBold'}}>{"Week : " + dataHolder.week}</Text>
                </View>
                <View style={{height: 1.5, marginLeft: 15, marginRight: 15, backgroundColor: 'lightgray', marginTop: 5}}></View>
                ...
                {this.state.isPause &&
                  <View style={{justifyContent: 'center', alignItems: 'center', position: 'relative'}}>
                      <Image source={{uri:dataHolder.video_poster}}
                ...
                {!this.state.isPause &&
                <Video
                    source={{uri: dataHolder.video_url}}
                    rate={1.0}
                    volume={1.0}
                    isMuted={false}
                    ref={videoplayer => {
    ...
    renderCommentSection(dataHolder, index) {
        return (
            <View key={index}
                ...
                }}>{this.state.commentOwn ? this.state.popupQuestion : dataHolder.question_placeholder}</Text>
                <Text style={{
                    color: 'black', fontFamily:
                        'PoppinsLight', marginLeft: 0,
                    fontSize: 12, marginTop: 3, marginRight: 7
                }}>{this.state.commentOwn ? this.state.commentText : dataHolder.user_actions.comment}
                </Text>
            </View>
        )
    }
